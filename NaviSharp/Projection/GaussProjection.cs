...
public class GaussProjection
{
    ...
    public EarthEllipsoid Ellipsoid { get; set; }
    public Angle LongitudeOfCentralMeridian { get; set; }

    #endregion Public Properties

    #region Public Methods

    public (double X, double Y) FromEllipsoidToGaussPlane(Angle latitude, Angle longitude)
    {
        var B = latitude.Rads;
        var sinB = Sin(B);
        var cosB = Cos(B);
        var N = Ellipsoid.N(B);
        var l = (longitude - LongitudeOfCentralMeridian).Rads;
        var e2_1 = Ellipsoid.E1 * Ellipsoid.E1;
        var e2_2 = Ellipsoid.E2 * Ellipsoid.E2;
        var t = Tan(B);
    ...
    public (Angle Latitude, Angle Longitude) FromGaussPlaneToEllipsoid(double x, double y)
    {
        var e2_1 = Ellipsoid.E1 * Ellipsoid.E1;
        var B_f0 = x / (Ellipsoid.A * _a * (1 - e2_1));
        var GetB_f1 = (double B_f0) => x / (Ellipsoid.A * _a * (1 - e2_1)) + 1 / _a * (_b / 2 * Sin(2 * B_f0)) - _c / 4 * Sin(4 * B_f0) + _d / 6 * Sin(6 * B_f0) - _e / 8 * Sin(8 * B_f0);
        var B_f1 = GetB_f1(B_f0);
        while (Abs(B_f1 - B_f0) >= FromDegrees(0, 0, 0.0001).Rads)
        {
            B_f0 = B_f1;
            B_f1 = GetB_f1(B_f0);
        }
        ...
        var L = LongitudeOfCentralMeridian.Rads + y / (N * Cos(B_f1)) * (1 - y * y / (6 * N * N) * (1 + eta2 + 2 * t * t) + Pow(y, 4) / (120 * Pow(N, 4)) * (5 + 6 * eta2 + 28 * t * t + 8 * eta2 * t * t + 24 * Pow(t, 4)));
        return (new(B), new(L));
    }
    ...
    {
        var e2 = Ellipsoid.E1 * Ellipsoid.E1;
        var e4 = e2 * e2;
        var e6 = e4 * e2;
        var e8 = e6 * e2;
        var e10 = e8 * e2;
